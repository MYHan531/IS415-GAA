...
event_types <- c("Battles", "Strategic Developments", "Violence against civilians", "Explosions/Remote violence")

# Loop through each quarter and event type
for (q in unique_quarters) {
  
  # Filter the data for the current quarter
  df_quarter <- myanmar_sf %>% filter(quarter == q)
  
  # Loop through each event type within the current quarter
  for (event in event_types) {
    ...
    df_name <- paste0("myanmar_sf_", gsub(" ", "_", q), "_", gsub("/", "", gsub(" ", "_", event)))
...
To perform spatio-temporal analysis, we need to create new `ppp` objects that include a temporal component. For this case, we'll use the disorder type as the temporal mark.

```{r}
# myanmar_quarter <- myanmar_sf %>% 
#   dplyr::select()
...
for (df_name in myanmar_sfs) {
  ...
  df <- get(df_name)
  # Create a quarter numeric mark
  df <- df %>%
    mutate(Quarter_num = as.numeric(gsub(".*Q", "", quarter)))
  
  # Convert to ppp object with Quarter_num as the mark
  ppp_object <- as.ppp(df %>% dplyr::select(Quarter_num))
  
  # Assign the ppp object to a variable
  assign(gsub("sf", "ppp_temp", df_name), ppp_object)
}
myanmar_ppps2 = ls(pattern = "myanmar_ppp_temp")
```
...
```{r}
# List to store KDE results
kde_results <- list()

# Loop through each quarterly ppp object to compute spatio-temporal KDE
for (ppp_name in myanmar_ppps2) {
  
  ppp_object <- get(ppp_name)
  
  # Ensure the temporal mark is numeric
  if (!is.numeric(marks(ppp_object))) {
    stop("Temporal mark in the ppp object must be numeric.")
  }
  
  # Compute spatio-temporal KDE
  st_kde <- spattemp.density(ppp_object)
  
  # Store the KDE result in a list
  kde_results[[ppp_name]] <- st_kde
  
  # Plot the spatio-temporal KDE for the current quarter
  plot(st_kde, main = paste("ST-KDE for", ppp_name))
}
```
